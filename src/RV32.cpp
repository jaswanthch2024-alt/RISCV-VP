...
namespace riscv_tlm {
    ...
    bool CPURV32::CPU_step() {
        last_mem_access = false; // reset
        if (dmi_ptr_valid) {
            std::memcpy(&INSTR, dmi_ptr + register_bank->getPC(), 4);
            if (m_qk) { m_qk->inc(sc_core::SC_ZERO_TIME); }
        } else {
            sc_core::sc_time delay = sc_core::SC_ZERO_TIME;
            tlm::tlm_dmi dmi_data; trans.set_address(register_bank->getPC()); instr_bus->b_transport(trans, delay);
            if (trans.is_response_error()) { SC_REPORT_ERROR("CPU base", "Read memory"); }
            if (trans.is_dmi_allowed()) { dmi_ptr_valid = instr_bus->get_direct_mem_ptr(trans, dmi_data); if (dmi_ptr_valid) { dmi_ptr = dmi_data.get_dmi_ptr(); } }
            if (m_qk) { m_qk->inc(delay); if (m_qk->need_sync()) m_qk->sync(); }
        }
        ...
        if (deco != OP_ERROR) { auto PC_not_affected = base_inst->exec_instruction(inst, &breakpoint, deco); if (PC_not_affected) { register_bank->incPC(); } }

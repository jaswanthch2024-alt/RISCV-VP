...
/**
 * @file CPU_P32_2_Cycle.cpp
 * @brief 2-Stage Pipelined RISC-V 32-bit CPU - Cycle-Accurate Implementation
 * 
 * True cycle-accurate simulation with:
 * - Clock-edge driven pipeline
 * - Configurable instruction latencies
 * - Precise stall and hazard modeling
 */
...
CPURV32P2_Cycle::CPURV32P2_Cycle(sc_core::sc_module_name const& name, BaseType PC, bool debug)
    : CPU(name, debug) {
    ...
    // Register cycle thread
    SC_THREAD(cycle_thread);

    logger->info("Created CPURV32P2_Cycle (cycle-accurate) CPU for VP");
    std::cout << "Created CPURV32P2_Cycle (cycle-accurate 2-stage pipelined) CPU" << std::endl;
}

CPURV32P2_Cycle::~CPURV32P2_Cycle() {
    printStats();
...
void CPURV32P2_Cycle::set_clock(sc_core::sc_clock* c) {
    clk = c;
    if (clk) {
...
void CPURV32P2_Cycle::printStats() const {
    std::cout << "\n========== Cycle-Accurate CPU Statistics ==========" << std::endl;
    ...
    std::cout << "---" << std::endl;
    std::cout << "Instruction Cycles:    " << stats.instruction_cycles << std::endl;
    std::cout << "Stall Cycles:          " << stats.stall_cycles << std::endl;
    std::cout << "Fetch Cycles:          " << stats.fetch_cycles << std::endl;
    std::cout << "Memory Cycles:         " << stats.memory_cycles << std::endl;
    std::cout << "Branch Penalty Cycles: " << stats.branch_penalty << std::endl;
    std::cout << "===================================================" << std::endl;
}
...
void CPURV32P2_Cycle::cycle_thread() {
    ...
    while (true) {
        // =====================================================================
        // RISING EDGE: Latch transfer + EX stage
        // =====================================================================
        on_posedge();
...
// =============================================================================
// Rising Edge: Transfer Latch + Execute
// =============================================================================

void CPURV32P2_Cycle::on_posedge() {
    stats.total_cycles++;
    
    // Process IRQ at cycle boundary
    cpu_process_IRQ();
...
// =============================================================================
// IF Stage: Fetch with Cycle-Accurate Latency
// =============================================================================

void CPURV32P2_Cycle::IF_stage() {
    ...
    if (fetch_instruction(current_pc, fetched_instr)) {
        ...
        // Speculatively increment PC
        if ((fetched_instr & 0x3) != 0x3) {
            register_bank->incPCby2();  // Compressed
        } else {
            register_bank->incPC();     // Normal
        }
...
// =============================================================================
// Fetch Instruction with Latency Modeling
// =============================================================================

bool CPURV32P2_Cycle::fetch_instruction(std::uint32_t pc, std::uint32_t& instruction) {
    // DMI fast path
    if (dmi_ptr_valid) {
    ...
    if (mem_state == MemState::IDLE) {
        ...
        // Perform the actual memory transaction
        ...
        if (trans.is_response_error()) {
            SC_REPORT_ERROR("CPURV32P2_Cycle", "Instruction fetch error");
            return false;
        }
...
uint32_t CPURV32P2_Cycle::get_instruction_latency(std::uint32_t instruction) {
    ...
    if (opcode == 0x33 && funct7 == 0x01) {
        if (funct3 < 4) {
            return latency.mul_latency;  // MUL, MULH, MULHSU, MULHU
        } else {
            return latency.div_latency;  // DIV, DIVU, REM, REMU
        }
    }
    
    // Load instructions
    if (opcode == 0x03) {
        return latency.load_latency;
    }
    
    // Store instructions
    if (opcode == 0x23) {
        return latency.store_latency;
    }
...
// =============================================================================
// EX Stage: Decode and Execute
// =============================================================================

bool CPURV32P2_Cycle::EX_stage() {
    ...
    // For multi-cycle instructions, add stall cycles
    if (instr_latency > 1) {
        ...
        if (c_deco != OP_C_ERROR) {
            ...
            if (m_deco != OP_M_ERROR) {
                ...
                if (a_deco != OP_A_ERROR) {
                    ...
                    std::cout << "Extension not implemented" << std::endl;
                    inst.dump();
                    base_inst->NOP();
                }
            }
...
// =============================================================================
// CPU_step (for debug mode)
// =============================================================================

bool CPURV32P2_Cycle::CPU_step() {
    on_posedge();
    sc_core::wait(clock_period / 2);
    on_negedge();
    sc_core::wait(clock_period / 2);
...
bool CPURV32P2_Cycle::cpu_process_IRQ() {
    BaseType csr_temp;
    ...
    if (interrupt) {
        ...
        if ((csr_temp & MIP_MEIP) == 0) {
            ...
            if_ex_latch.valid = false;
            if_ex_latch_next.valid = false;
            stats.stall_cycles += 2;  // IRQ latency
...
void CPURV32P2_Cycle::call_interrupt(tlm::tlm_generic_payload &m_trans, sc_core::sc_time &delay) {
    interrupt = true;
    memcpy(&int_cause, m_trans.get_data_ptr(), sizeof(BaseType));
    delay = sc_core::SC_ZERO_TIME;
}

std::uint64_t CPURV32P2_Cycle::getStartDumpAddress() {
    return register_bank->getValue(Registers<std::uint32_t>::t0);
}

std::uint64_t CPURV32P2_Cycle::getEndDumpAddress() {

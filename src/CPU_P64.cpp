...
CPURV64P::CPURV64P(sc_core::sc_module_name const& name, BaseType PC, bool debug)
    : CPU(name, debug) {
    ...
    logger->info("Created CPURV64P (pipelined) CPU");
    std::cout << "Created CPURV64P (pipelined) CPU" << std::endl;
}
...
void CPURV64P::stageIF() {
    ...
    if (dmi_ptr_valid) {
        ...
        if (trans.is_response_error()) {
            SC_REPORT_ERROR("CPURV64P", "Instruction fetch error");
        }
...
void CPURV64P::stageWB(bool &breakpoint) {
    if (!if_id.valid) {
        breakpoint = false;
        return;
    }
...
void CPURV64P::call_interrupt(tlm::tlm_generic_payload &m_trans, sc_core::sc_time &delay) {
    interrupt = true;
    memcpy(&int_cause, m_trans.get_data_ptr(), sizeof(BaseType));
    delay = sc_core::SC_ZERO_TIME;
}

std::uint64_t CPURV64P::getStartDumpAddress() {
    return register_bank->getValue(Registers<std::uint64_t>::t0);
}

std::uint64_t CPURV64P::getEndDumpAddress() {

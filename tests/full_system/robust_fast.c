...
// Reduced-scale robust test for faster execution while keeping all features

// Inline types (no libc headers needed)
...
static inline uint32_t read_mstatus(){ uint32_t v; __asm__ volatile("csrr %0,mstatus":"=r"(v)); return v; }
static inline void     write_mstatus(uint32_t v){ __asm__ volatile("csrw mstatus,%0"::"r"(v)); }
static inline void     write_mtvec(uint32_t v){ __asm__ volatile("csrw mtvec,%0"::"r"(v)); }
static inline void     write_mie(uint32_t v){ __asm__ volatile("csrw mie,%0"::"r"(v)); }
static inline uint32_t read_mcycle(){ uint32_t v; __asm__ volatile("csrr %0,mcycle":"=r"(v)); return v; }
static inline uint32_t read_minstret(){ uint32_t v; __asm__ volatile("csrr %0,minstret":"=r"(v)); return v; }
...
static void setup_timer_irq(){
    ...
    write_mie(1u << 7);
    write_mstatus(read_mstatus() | 0x8u);
    uint64_t now = ((uint64_t)MTIME_HI << 32) | MTIME_LO;
    uint64_t next = now + 8000ull;
    MTIMECMP_LO = (uint32_t)(next & 0xFFFFFFFFu);
    MTIMECMP_HI = (uint32_t)(next >> 32);
...
// REDUCED workload sizes for faster execution
#define BUF_WORDS  256   // was 4096
#define DMA_WORDS  128   // was 2048
...
int main(){
    ...
    // REDUCED compute loop (was 800)
    volatile uint32_t atomic_counter = 0;
    uint64_t accum = 0;
    for(uint32_t outer=0; outer < 50; ++outer){
        // Arithmetic mix (M extension)
